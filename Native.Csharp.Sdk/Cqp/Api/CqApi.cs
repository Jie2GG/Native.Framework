...
namespace Native.Csharp.Sdk.Cqp.Api
{
	public class CqApi
	{
        ...
        /// <param name="emojiDeserialize">还原由酷Q转译的原始Emoji</param>
        /// <returns><see cref="MessageContent"/></returns>
        public IEnumerable<MessageContent> ToMessageContent(string message, bool emojiDeserialize = false)
        {
            const string sCqCommandHeader = "[CQ:";
            const string sCqCommandFooter = "]";
            int index_processing = 0;
            List<MessageContent> messageContent = new List<MessageContent>();
            Dictionary<int, MessageContent> dic = new Dictionary<int, MessageContent>();
            Dictionary<int, int> dicRef = new Dictionary<int, int>();

            if (emojiDeserialize)
            {
                message = Emoji.DeserializeObject(message);
            }

            //特殊事件类消息
            if (message.StartsWith("&#91;视频&#93;你的QQ暂不支持查看视频短片，请升级到最新版本后查看。"))
            {
                messageContent.Add(new MessageContent { Type = ContentType.Video });
                return messageContent;
            }
            else if (message.StartsWith("&#91;群公告&#93;请使用新版QQ进行查看。"))
            {
                messageContent.Add(new MessageContent { Type = ContentType.GroupNotify });
                return messageContent;
            }
            else if (message.StartsWith("&#91;送礼物&#93;"))
            {
                System.Text.RegularExpressions.Match m = new System.Text.RegularExpressions.Regex(@"&#91;送礼物&#93; 为\[CQ:at,qq=([0-9]{6,15})\] 送出 (.*)").Match(message);
                if (m.Success)
                {
                    messageContent.Add(new MessageContent { Type = ContentType.Gift , TargetId = m.Groups[1].Value , Content = m.Groups[2].Value });
                    return messageContent;
                }
            }
            else if (message.StartsWith("&#91;QQ红包&#93;"))
            {
                System.Text.RegularExpressions.Match m = new System.Text.RegularExpressions.Regex(@"&#91;QQ红包&#93;我发了一个“(.*)”，请使用手机QQ").Match(message);
                if (m.Success)
                {
                    messageContent.Add(new MessageContent { Type = ContentType.Hb , Content = m.Groups[1].Value });
                    return messageContent;
                }
            }
            else if (message.StartsWith("&#91;闪照&#93;请使用新版手机QQ查看闪照。"))
            {
                messageContent.Add(new MessageContent { Type = ContentType.ShiningImage });
                return messageContent;
            }

            while (message.IndexOf(sCqCommandHeader, index_processing) > -1)
            {
                int tmp_start = message.IndexOf(sCqCommandHeader, index_processing);
                int tmp_end = message.IndexOf(sCqCommandFooter, index_processing + 1);
                int length = tmp_end - tmp_start + sCqCommandFooter.Length;
                if (tmp_end < 1) { break; }
                if (length < 1) { break; }
                if (message.Length < tmp_end) { break; }

                IEnumerable<string> tmpList = message.Substring(tmp_start + sCqCommandHeader.Length, (tmp_end - tmp_start - sCqCommandHeader.Length)).Split(',');
                string function = tmpList?.FirstOrDefault();
                Dictionary<string, string> param = new Dictionary<string, string>();
                tmpList?.Skip(1)?.Where(s => s.Contains("=")).ToList().ForEach(f => param.Add(f.Split('=')[0], f.Split('=')[1]));
                ...
                {
                    case "face":
                        Type = ContentType.Face;
                        param.TryGetValue("id", out TargetID);
                        break;
                    case "bface":
                        Type = ContentType.BFace;
                        param.TryGetValue("p", out TypeID);
                        param.TryGetValue("id", out TargetID);
                        break;
                    case "emoji":
                        ...
                        {
                            int unicode = int.Parse(TargetID);
                    ...
                    case "sface":
                        ...
                        break;
                    case "shake":
                        Type = ContentType.Shake;
                        break;
                    case "image":
                        Type = ContentType.Image;
                        param.TryGetValue("destruct", out string destruct);
                        if (destruct == "true") { Type = ContentType.ShiningImage; }
                        param.TryGetValue("file", out TargetUrl);
                        break;
                    case "record":
                        Type = ContentType.Record;
                        param.TryGetValue("file", out TargetUrl);

                        if (param.TryGetValue("magic", out TypeID))
                        {
                            if (TypeID == "true")
                            {
                                Type = ContentType.Record_Magic;
                                break;
                            }
                        }
                        if (param.TryGetValue("hb", out TypeID))
                        {
                            if (TypeID == "true")
                            {
                                Type = ContentType.Record_Magic;
                    ...
                    case "share":
                        ...
                        param.TryGetValue("url", out TargetUrl);
                        param.TryGetValue("title", out TargetTitle);
                        param.TryGetValue("image", out TargetID);
                        break;
                    case "music":
                        Type = ContentType.Music;
                        param.TryGetValue("type", out TypeID);
                        param.TryGetValue("id", out TargetID);
                        break;
                    case "dice":
                        ...
                        break;
                    case "sign":
                        Type = ContentType.Sign;
                        param.TryGetValue("location", out Content);
                        param.TryGetValue("title", out TargetTitle);
                        param.TryGetValue("image", out TargetUrl);
                        break;
                    case "show":
                    ...
                    case "at":
                        Type = ContentType.At;
                        param.TryGetValue("qq", out TargetID);
                        if(TargetID == "all")
                        {
                            TargetID = "-1";
                        }
                    ...
                    case "hb":
                        ...
                        break;
                    case "rich":
                        Type = ContentType.Rich;
                        param.TryGetValue("url", out TargetUrl);
                        param.TryGetValue("text", out Content);
                        break;
                    case "contact":
                        param.TryGetValue("id", out TargetID);
                        param.TryGetValue("type", out TypeID);
                        if (TypeID == "qq")
                        {
                            Type = ContentType.Contact_Qq;
                        }
                        else
                        {
                            Type = ContentType.Contact_Group;
                        }
                        break;
                }

                dic.Add(tmp_start, new MessageContent
                {
                    Type = Type,
                    TypeId = TypeID,
                    TargetId = TargetID,
                ...
                dicRef.Add(tmp_start, length);
                index_processing = tmp_end;
            }
            ...
            if (dicRef.Count == 0)
            {
                messageContent.Add(new MessageContent() { Type = ContentType.Text, Content = this.CqCode_UnTrope(message) });
            }
            else
            {
                for (int point = 0; point <= message.Length; point++)
                {
                    if (dicRef.TryGetValue(point, out int length) || point == message.Length)
                    {
                        if (sb.Length > 0)
                        {
                            messageContent.Add(new MessageContent() { Type = ContentType.Text, Content = this.CqCode_UnTrope(sb.ToString()) });
                        }

                        if (dic.TryGetValue(point, out MessageContent _messageContent))
                        {
                            messageContent.Add(dic[point]);
                            point += length - 1;
                        }
